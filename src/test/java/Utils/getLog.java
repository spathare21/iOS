...
public class getLog {
    ...
    public static void getlog(String udid) throws IOException {
        ...
        final String command = "idevicesyslog -u " + udid;
    ...
    public static void log_save() throws IOException {
        System.out.println(" now we will save log");
        BufferedReader stdInput = new BufferedReader(new
        ...
        String logs;
        int count = 0;
        while((logs = sc.nextLine())!= null)
        {
            if (logs.contains("Notification Received"))
            {
                FileUtils.writeStringToFile(file, logs  +  "  counting:" + count +   "  \n ");
                count++;
            }
        }
